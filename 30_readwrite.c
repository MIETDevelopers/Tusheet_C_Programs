...
struct s {
 ...
 int main(){
	...
	fptr=fopen("E:\\Softwares\\C_Programs\\\\030_ReadWriteFile.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
		printf("Enter Text: ");
	...
	fclose(fptr);
	fptr=fopen("E:\\Softwares\\C_Programs\\\\030_ReadWriteFile.txt","rb");
